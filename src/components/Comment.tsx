...
export default function Comment({
  ...
  const [isCommentBeingEdited, setIsCommentBeingEdited] =
  ...
  return (
    <div
      key={item.id}
      className="bg-white rounded-[0.5rem] p-6 flex gap-6 mb-4 max-md:flex-col-reverse"
    >
      {/* div with the score button and the hidden edit, delete and reply buttons*/}
      <div className="flex justify-between">
        ...
        <div className="bg-grey-100 flex md:flex-col items-center justify-between md:p-3 max-md:py-2 max-md:px-3 rounded-[0.5rem] md:h-24 md:min-w-10">
          ...
          <button
            onClick={() => {
              if (!alreadyVotedPlus) {
                const commentIndex = userComments.comments.indexOf(item);
                const currentComment = {
                  ...userComments.comments[commentIndex],
                };
                const updatedScore = {
                  ...currentComment,
                  score: currentComment.score + 1,
                };
                const updatedComments = userComments.comments.map(
                  (comment: any) => {
                    return comment.id === item.id ? updatedScore : comment;
                  }
                );
          ...
          >
            <svg width="11" height="11" xmlns="http://www.w3.org/2000/svg">
              <path
                ...
                fill="#C5C6EF"
                className={`${alreadyVotedPlus ? "fill-purple-600" : ""}`}
              />
            </svg>
          </button>
          ...
          >
            <svg width="11" height="3" xmlns="http://www.w3.org/2000/svg">
              <path
                ...
                fill="#C5C6EF"
                className={`${alreadyVotedMinus ? "fill-purple-600" : ""}`}
              />
            </svg>
          </button>
        </div>

        {/* this div will appear if the screen width is smaller than 770px */}
        <div className="md:hidden max-md:flex">
          {item.user.username === "juliusomo" ? (
            <div className="hidden gap-5 *:flex *:items-center *:gap-2 *:font-semibold max-md:flex">
              {/* delete button */}
              <button
                onClick={() => {
        ...
        {item.user.username === "juliusomo" && (
          <div
            className={`flex w-full justify-end mt-4 ${
              isCommentBeingEdited ? "flex" : "hidden"
            }`}
          >
            {/* update button */}
            <button
